...
class DatasetController():
    ...
    def execute(self):
        if len(self.y_train) == 0:
            ...
            if self.cyclicalPattern: self.CyclicalPattern()
            self.df = self.StripDataset(df=self.df)
    ...
    def ResampleTime(self, df:pl.DataFrame, features:list) -> pd.DataFrame:
        pd_df = df.to_pandas()
        pd_df = pd_df.drop(self.segmentFeature, axis=1)
        inserted_row = [pd_df.iloc[0][self.dateFeature].floor(f'{self.resample}min')]
        ...
        pd_df = pd_df.set_index(self.dateFeature).asfreq(f'{self.resample}min')
        pd_df.reset_index(inplace=True)

        df = df.groupby_dynamic(self.dateFeature, every=f"{self.resample}m").agg(pl.col(self.targetFeatures).mean())
        df = df.to_pandas()
        df = df.set_index(self.dateFeature).asfreq(f'{self.resample}min')
        df.reset_index(inplace=True)
        pd_df = pd_df.combine_first(df)

        pd_df = pl.from_pandas(pd_df)
        pd_df = pd_df[features].to_pandas()
    ...
    #     self.df = self.ReduceMemoryUsage(df=self.df, info=False)
    #     self.SortDataset()
    #     self.df.write_csv(save_dir / 'data_processed.csv')

    #     LIST_FEATURE = [*self.trainFeatures, self.targetFeatures]
    #     if self.segmentFeature in LIST_FEATURE: LIST_FEATURE.remove(self.segmentFeature)
    ...
    #     with self.ProgressBar() as progress:
    #         for id_ in progress.track(sorted(int(i) for i in self.df[self.segmentFeature].unique()), description='Splitting data'):
    #             x_train = []
    #             y_train = []
    #             x_val = []
    #             y_val = []
    #             x_test = []
    #             y_test = []
    #             sub_pl_dataframe = self.df.filter(pl.col(self.segmentFeature)==id_)

    #             pd_df = self.ResampleTime(df=sub_pl_dataframe, features=[*LIST_FEATURE, self.dateFeature])


    #             for ft in LIST_FEATURE:
    #                 for i in range(1, lag_window):
    #                     pd_df = pd_df.merge(pd_df[ft].shift(i, freq=f'{self.resample}min'),
    #                                         left_index=True,
    #                                         right_index=True,
    #                                         how='left',
    #                                         suffixes=('', f'_lag_{i}'))
    ...
    #             x_train.extend(np.swapaxes(input_feature[0:train_idx].reshape((train_num, -1, lag)), 1,2 ))
    #             y_train.extend(target_feature[0:train_idx] )
    #             x_val.extend(np.swapaxes(input_feature[train_idx:val_id].reshape((val_num, -1, lag)), 1,2 ) )
    #             y_val.extend(target_feature[train_idx:val_id] )
    #             x_test.extend(np.swapaxes(input_feature[val_id:].reshape((test_num, -1, lag)), 1,2 ) )
    ...
    def SplittingData(self, splitRatio, lag, ahead, offset, multimodels=False, low_memory=False, save_dir=None):
        ...
        xt  = save_dir / 'xtest.npy'
        yt  = save_dir / 'ytest.npy'
        xtr_writer =  NpyFileAppend(xtr, delete_if_exists=True)
        ytr_writer =  NpyFileAppend(ytr, delete_if_exists=True)
        xv_writer =  NpyFileAppend(xv, delete_if_exists=True)
        yv_writer =  NpyFileAppend(yv, delete_if_exists=True)
        xt_writer =  NpyFileAppend(xt, delete_if_exists=True)
        yt_writer =  NpyFileAppend(yt, delete_if_exists=True)

        normalized_features = ['speed']
        if self.normalization:
            writers = {} 
            for ele in normalized_features:
                writers[ele] = NpyFileAppend(save_dir / f'{ele}.npy')
        # print(ytr_writer, writers['speed'])
        # exit()
        ...
        with self.ProgressBar() as progress:
            for id_ in progress.track(sorted(int(i) for i in self.df[self.segmentFeature].unique()), description='Splitting data'):
                ...
                pd_df.reset_index(inplace=True)
                pd_df['index_col'] = pd_df.index
                pd_df_na = pd_df.dropna()
                # print(len(pd_df_na['index_col'].to_list()))
                # idxx = int(pd_df_na['index_col'].shape[0]*splitRatio[0])
                # print(idxx)
                # print(pd_df_na['index_col'].to_list()[1297])
                # print(pd_df_na['index_col'].to_list()[idxx])
                # end_train_index = pd_df_na['index_col'].to_list()[idxx]
                if pd_df_na.shape[0] == 0:
                    invalid.append(id_)
                    np.save(save_dir / 'invalid.npy', invalid)
                    continue

                end_train_index = pd_df_na['index_col'].to_list()[int(pd_df_na['index_col'].shape[0]*splitRatio[0])]
                # exit()
                # print('dsdsada')
                # print(pd_df[[*LIST_FEATURE, self.dateFeature]][0:end_train_index])
                # print('dsdsada')
                pd_df = pd_df[[*LIST_FEATURE, self.dateFeature]][0:end_train_index]
                pd_df = pd_df.set_index(self.dateFeature)
                if self.polarsFilling is not None:
                    if self.polarsFilling in ['nearest', 'zero', 'slinear', 'quadratic', 'cubic', 'barycentric', 'polynomial']:
                        for ele in LIST_FEATURE:
                            pd_df[ele] = pd_df[ele].interpolate(method=self.polarsFilling)

                    if self.polarsFilling == 'min':
                        for ele in LIST_FEATURE:
                            pd_df[ele] = pd_df[ele].fillna(value=pd_df[ele].min(numeric_only=True).round(1))

                    if self.polarsFilling == 'max':
                        for ele in LIST_FEATURE:
                            pd_df[ele] = pd_df[ele].fillna(value=pd_df[ele].max(numeric_only=True).round(1))

                    if self.polarsFilling == 'mean':
                        for ele in LIST_FEATURE:
                            pd_df[ele] = pd_df[ele].fillna(value=pd_df[ele].mean(numeric_only=True).round(1))
                    
                    if self.polarsFilling in ['bfill', 'ffill']:
                        for ele in LIST_FEATURE:
                            pd_df[ele] = pd_df[ele].fillna(method=self.polarsFilling)

                    pd_df = pd_df.reset_index().rename(columns={'index': self.dateFeature})
                    pd_df[self.dateFeature] = pd.to_datetime(pd_df[self.dateFeature])
                    mask1 = (pd_df[self.dateFeature].dt.hour < 4)
                    mask2 = ((pd_df[self.dateFeature].dt.hour == 22) & (pd_df[self.dateFeature].dt.minute >= 5))
                    mask3 = ((pd_df[self.dateFeature].dt.hour == 23))
                    pd_df = pd_df[~(mask1 | mask2 | mask3)]
                    # pd_df = pd_df[((pd_df[self.dateFeature].dt.hour >= 4) & 
                    #                ((pd_df[self.dateFeature].dt.hour <= 22) & (pd_df[self.dateFeature].dt.minute < 5)))]
                    pd_df = pd_df.reset_index(drop=True)
                    pd_df.set_index(self.dateFeature, inplace=True)

                    pd_df.to_csv(save_dir / f'{id_}.csv')
                # ----------------------------------------------------
                # actual train 
                for ft in LIST_FEATURE:
                    for i in range(1, lag_window):
                ...
                if self.normalization:
                    for ele in normalized_features:
                        # print(pd_df[ele])
                        writers[ele].append(np.array(pd_df[ele].to_list()))

                train_input_feature = pd_df.drop(columns=LIST_FEATURE).to_numpy()
                train_target_feature = pd_df[self.targetFeatures].to_numpy()
                train_num = train_target_feature.shape[0]
                #-----------------------------------------------------

                # pd_df_na = pd_df_na.dropna()
                pd_df_na = pd_df_na[FINAL_LIST_FEATURE]
                input_feature =  pd_df_na.drop(columns=LIST_FEATURE).to_numpy()
                target_feature = pd_df_na[self.targetFeatures].to_numpy()
                
                num_samples = len(target_feature)
                nan_train_num = train_idx = int(num_samples*splitRatio[0])
                val_id = int(num_samples*(splitRatio[0] + splitRatio[1]))
                val_num = val_id-nan_train_num
                test_num = num_samples-(nan_train_num+val_num)
                
                if any([train_input_feature.size == 0, 
                        train_target_feature.size == 0, 
                        input_feature[train_idx:val_id].size == 0, 
                        target_feature[train_idx:val_id].size == 0, 
                        input_feature[val_id:].size == 0, 
                        target_feature[val_id:].size == 0]):
                ...
                x_train.extend(np.swapaxes(train_input_feature.reshape((train_num, -1, lag)), 1,2 ))
                y_train.extend(train_target_feature )
                # x_train.extend(np.swapaxes(input_feature[0:train_idx].reshape((nan_train_num, -1, lag)), 1,2 ))
                # y_train.extend(target_feature[0:train_idx] )
                x_val.extend(np.swapaxes(input_feature[train_idx:val_id].reshape((val_num, -1, lag)), 1,2 ) )
                y_val.extend(target_feature[train_idx:val_id] )
                x_test.extend(np.swapaxes(input_feature[val_id:].reshape((test_num, -1, lag)), 1,2 ) )
                ...
                # if os.path.exists(xtr): print('berfore: ', np.load(file=xtr, mmap_mode='r+').shape)
                xtr_writer.append(x_train)
                # print('append: ', x_train.shape)
                # print('after: ', np.load(file=xtr, mmap_mode='r+').shape, end='\n\n')
            ...
            if len(self.df[self.segmentFeature].unique()) < 500:
                self.X_train = np.load(file=xtr, mmap_mode='r+')
                self.y_train = np.load(file=ytr, mmap_mode='r+')
                self.X_val = np.load(file=xv, mmap_mode='r')
                self.y_val = np.load(file=yv, mmap_mode='r')
                ...
                self.X_test = np.load(file=xt, mmap_mode='r')
                self.y_test = np.load(file=yt, mmap_mode='r')
            # print('after: ', np.load(file=ytr, mmap_mode='r+').shape, end='\n\n')
            # exit()


            if self.normalization:
                self.scaler = []
                for idx, ele in enumerate(normalized_features):
                    a = np.load(save_dir / f'{ele}.npy', mmap_mode='r')
                    mean = a.mean()
                    std = a.std()
                    # print(mean.shape, std.shape, a.shape, self.X_train.shape)
                    if ele in self.targetFeatures:
                        self.scaler.append({'mean' : mean,
                                            'std' : std})
                        self.y_train = (self.y_train - mean) / std
                    self.X_train[:, :, -(idx+1)] = (self.X_train[:, :, -(idx+1)] - mean) / std
